...
MLModel::MLModel()
{
    // ADD: get model files from env
    char* _nameFile = std::getenv("NAME_FILE");
    char* _cfgFile = std::getenv("CFG_FILE");
    char* _weightsFile = std::getenv("WEIGHTS_FILE");

    namesFile = (_nameFile == NULL) ? "coco.names" : _nameFile;
    cfgFile = (_cfgFile == NULL) ? "yolov3.cfg" : _cfgFile;
    weightsFile = (_weightsFile == NULL) ?  "yolov3.weights" : _weightsFile;

    std::cout << "Model files: " << namesFile << ", " << cfgFile << ", " << weightsFile << std::endl;
